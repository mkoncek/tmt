...
rlJournalStart
    ...
    rlPhaseStartTest "Truncated"
        for v in '' '-v'; do
          rlRun -s "tmt run --id $run --all \
            provision $v --how local \
            execute --how tmt --script true \
            prepare --how shell --script 'for i in {1..101}; do echo OUT-\$i; done; false'" "$ECODE"
    ...
    rlPhaseStartTest "Not Truncated"
        ...
        rlRun -s "tmt run -v --id $run --all \
          provision $v --how local \
          execute --how tmt --script true \
          prepare --how shell --script 'for i in {1..101}; do echo OUT-\$i; done; false'" "$ECODE"

        rlAssertGrep 'stdout (101 lines)' $rlRun_LOG -F
        rlAssertGrep 'OUT-101' $rlRun_LOG -F

        # Prepare is in verbose mode
        rlRun -s "tmt run --id $run --all \
          provision --how local \
          execute --how tmt --script true \
          prepare -v --how shell --script 'for i in {1..101}; do echo OUT-\$i; done; false'" "$ECODE"
